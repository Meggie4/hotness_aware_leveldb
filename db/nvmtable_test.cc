...
TEST(NVMTableTest, Simple) {
    ...
    for(int i = 0; i < chunk_num; i++){
        std::string chunklogfile = chunkLogFileName(nvm_path, file_number);
        file_number++;
        fprintf(stderr, "chunklogfile:%s\n", chunklogfile.c_str());
        size_t filesize = (64 << 10)<< 10;
        chunkLog* ckg = new chunkLog(&chunklogfile, filesize, false);
        cklogs.push_back(ckg);
        ...
        ////////arenanvm
        std::string chunkindexfile = chunkIndexFileName(nvm_path, file_number);
        file_number++;
        size_t chunkindexfile_size = (4 << 10) << 10;
        ArenaNVM* arena = new ArenaNVM(&chunkindexfile, chunkindexfile_size, false);
    ...
    //ckTbl->Ref();
    NVMTable* nvmTbl = new NVMTable(cmp, arenas, cklogs, false);
    nvmTbl->Ref(); 
    int num = 100;
    for(int i = 0; i < num; i++){
        size_t kvlen;
        ...
        //fprintf(stderr, "kv1:%p, before table add, user_key:%s, len:%d\n", 
          //      kv, Slice(key_ptr, key_length - 8).ToString().c_str(), 
            //    key_length - 8);
        nvmTbl->Add(kv, Slice(key_ptr, key_length - 8));
        ...
        std::string value;
        Status s;
        nvmTbl->Get(Slice(key, key_len), &value, &s, 10000);
        ...
        //////////////contains 
        ASSERT_TRUE(nvmTbl->Contains(Slice(key, key_len), 10000));
        ...
        if(i >= 0){
            ...
            //std::cout<<"before update, key_num:"<<nvmTbl->getKeyNum()<<std::endl;
            nvmTbl->Add(newkv.data(), newkey);
            //std::cout<<"after update, key_num:"<<nvmTbl->getKeyNum()<<std::endl;
            nvmTbl->Get(newkey, &value, &s, 100001);
            ...
            nvmTbl->Add(deletekv.data(), newkey);
            nvmTbl->Get(newkey, &value, &s, 10003);
    ...
    Iterator* ckiter = nvmTbl->getchunkTableIterator(0);
    ckiter->SeekToFirst();
    while(ckiter->Valid()){
        std::cout<<"key: "<<ExtractUserKey(ckiter->key()).ToString()<<", value: "
            <<ckiter->value().ToString()<<std::endl;
        ckiter->Next();
    }
